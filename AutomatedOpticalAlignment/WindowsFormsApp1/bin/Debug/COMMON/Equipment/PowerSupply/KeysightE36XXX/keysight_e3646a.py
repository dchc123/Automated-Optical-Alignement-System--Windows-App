...
For the top level API: See :py:class:`.KeysightE3646A`
::

    >>> from CLI.Equipment.PowerSupply.KeysightE36XXX.keysight_e3646a import KeysightE3646A
    >>> ps = KeysightE3646A('GPIB1::23::INSTR')
    >>> ps.connect()

For channel level API:
:py:class:`.KeysightE3646AChannel`
::
...
class KeysightE3646A(KeysightE36XXX):
    """
    Keysight E3646A power supply driver.
    """
    def __init__(self, address, interface=None, dummy_mode=False, **kwargs):
        ...
        """:type: list of KeysightE3646AChannel"""
        self._memory = _KeysightE36XXXMemory(interface=interface, dummy_mode=dummy_mode)
        self.channel.append(KeysightE3646AChannel(channel_number=1, memory=self._memory,interface=interface,
                                                  dummy_mode=dummy_mode))
        self.channel.append(KeysightE3646AChannel(channel_number=2, memory=self._memory, interface=interface,
                                                  dummy_mode=dummy_mode))


class KeysightE3646AChannel(KeysightE36XXXChannel):
    """
    Keysight E3646A Channel
    """
    def __init__(self, channel_number, memory, interface, dummy_mode, **kwargs):
        ...
        volt_threshold = 8.24
        curr_threshold = 1.545
        self.voltage = KeysightE3646AVoltBlock(channel_number=channel_number, range_values=temp_range_values,
                                               range_threshold=volt_threshold, memory=memory,
                                               interface=interface, dummy_mode=dummy_mode,
                                               name=self.name+'VoltageMeasure')
        self.current = KeysightE3646ACurrBlock(channel_number=channel_number, range_values=temp_range_values,
                                               range_threshold=curr_threshold, memory=memory,
                                               interface=interface, dummy_mode=dummy_mode,
                                               name=self.name+'CurrentMeasure')


class KeysightE3646AVoltBlock(KeysightE36XXXVoltBlock):
    """
    Keysight E3646A voltage sub block
    """
...
class KeysightE3646ACurrBlock(KeysightE36XXXCurrBlock):
    """
    Keysight E3646A current sub block
    """
    CAPABILITY = {'current': {'min': 0.0, 'max': 3.09}}
