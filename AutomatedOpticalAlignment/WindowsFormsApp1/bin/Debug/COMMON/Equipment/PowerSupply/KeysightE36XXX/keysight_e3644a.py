...
For the top level API: See :py:class:`.KeysightE3644A`
::

    >>> from COMMON.Equipment.PowerSupply.KeysightE36XXX.keysight_e3644a import KeysightE3644A
    >>> ps = KeysightE3644A('GPIB1::23::INSTR')
    >>> ps.connect()

For channel level API:
:py:class:`.KeysightE3644AChannel`
::
...
class _KeysightE3644AMemory(_KeysightE36XXXMemory):
    """
...
class KeysightE3644A(KeysightE36XXX):
    """
    Keysight E3644A power supply driver.
    """
    def __init__(self, address, interface=None, dummy_mode=False, **kwargs):
        ...
        """:type: list of KeysightE3644AChannel"""
        self._memory = _KeysightE3644AMemory(interface=interface, dummy_mode=dummy_mode)
        self.channel.append(KeysightE3644AChannel(channel_number=1, memory=self._memory,
                                                  interface=interface, dummy_mode=dummy_mode))


class KeysightE3644AChannel(KeysightE36XXXChannel):
    """
    Keysight E3644A Channel
    """
    def __init__(self, channel_number, memory, interface, dummy_mode, **kwargs):
        ...
        volt_threshold = 8.24
        curr_threshold = 8.24
        self.voltage = KeysightE3644AVoltBlock(channel_number=channel_number, range_values=temp_range_values,
                                               range_threshold=volt_threshold, memory=memory,
                                               interface=interface, dummy_mode=dummy_mode,
                                               name=self.name+'VoltageMeasure')
        self.current = KeysightE3644ACurrBlock(channel_number=channel_number, range_values=temp_range_values,
                                               range_threshold=curr_threshold, memory=memory,
                                               interface=interface, dummy_mode=dummy_mode,
                                               name=self.name+'CurrentMeasure')


class KeysightE3644AVoltBlock(KeysightE36XXXVoltBlock):
    """
    Keysight E3644A voltage sub block
    """
...
class KeysightE3644ACurrBlock(KeysightE36XXXCurrBlock):
    """
    Keysight E3644A current sub block
    """
    CAPABILITY = {'current': {'min': 0.01, 'max': 8.24}}
