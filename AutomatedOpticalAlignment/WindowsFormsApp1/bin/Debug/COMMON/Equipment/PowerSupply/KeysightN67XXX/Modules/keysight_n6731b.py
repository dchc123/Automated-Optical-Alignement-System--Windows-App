...
:py:class:`.KeysightN6731BChannel`
::
    ...
    0.102
    >>> ps.channel[1].voltage.protection_level = 4
    >>> ps.channel[1].voltage.setpoint = 5
    >>> ps.channel[1].voltage.protection_tripped
    True
    >>> ps.channel[1].voltage.setpoint = 3.5
...
class KeysightN6731BChannel(KeysightN67XXXChannel):
    """
    Keysight N6731B Channel
    """
    ...
    def __init__(self, channel_number, channel_options, interface, dummy_mode, **kwargs):
        ...
        self.voltage = KeysightN6731BVoltBlock(channel_number=channel_number, channel_options=channel_options,
                                               interface=interface, dummy_mode=dummy_mode,
                                               name=self.name+'VoltageMeasure')
        self.current = KeysightN6731BCurrBlock(channel_number=channel_number, channel_options=channel_options,
                                               interface=interface, dummy_mode=dummy_mode,
                                               name=self.name+'CurrentMeasure')


class KeysightN6731BVoltBlock(KeysightN67XXXVoltBlock):
    """
    Keysight N6731B voltage sub block
    """

    CAPABILITY = {'voltage': {'min': 0.015, 'max': 5.1}}
...
class KeysightN6731BCurrBlock(KeysightN67XXXCurrBlock):
    """
    Keysight N6731B current sub block
    """

    CAPABILITY = {'current': {'min': 0.06, 'max': 10.2}}
