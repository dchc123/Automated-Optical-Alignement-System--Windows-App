...
:py:class:`.KeysightN6742BChannel`
::
...
class KeysightN6742BChannel(KeysightN67XXXChannel):
    """
    Keysight N6742B Channel
    """
    ...
    def __init__(self, channel_number, channel_options, interface, dummy_mode, **kwargs):
        ...
        self.voltage = KeysightN6742BVoltBlock(channel_number=channel_number, channel_options=channel_options,
                                               interface=interface, dummy_mode=dummy_mode,
                                               name=self.name+'VoltageMeasure')
        self.current = KeysightN6742BCurrBlock(channel_number=channel_number, channel_options=channel_options,
                                               interface=interface, dummy_mode=dummy_mode,
                                               name=self.name+'CurrentMeasure')


class KeysightN6742BVoltBlock(KeysightN67XXXVoltBlock):
    """
    Keysight N6742B voltage sub block
    """
...
class KeysightN6742BCurrBlock(KeysightN67XXXCurrBlock):
    """
    Keysight N6742B current sub block
    """

    CAPABILITY = {'current': {'min': 0.04, 'max': 12.75}}
