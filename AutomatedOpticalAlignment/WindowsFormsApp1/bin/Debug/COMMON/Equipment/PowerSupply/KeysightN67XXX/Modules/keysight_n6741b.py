...
:py:class:`.KeysightN6741BChannel`
::
...
class KeysightN6741BChannel(KeysightN67XXXChannel):
    """
    Keysight N6741B Channel
    """
    ...
    def __init__(self, channel_number, channel_options, interface, dummy_mode, **kwargs):
        ...
        self.voltage = KeysightN6741BVoltBlock(channel_number=channel_number, channel_options=channel_options,
                                               interface=interface, dummy_mode=dummy_mode,
                                               name=self.name+'VoltageMeasure')
        self.current = KeysightN6741BCurrBlock(channel_number=channel_number, channel_options=channel_options,
                                               interface=interface, dummy_mode=dummy_mode,
                                               name=self.name+'CurrentMeasure')


class KeysightN6741BVoltBlock(KeysightN67XXXVoltBlock):
    """
    Keysight N6741B voltage sub block
    """
...
class KeysightN6741BCurrBlock(KeysightN67XXXCurrBlock):
    """
    Keysight N6741B current sub block
    """

    CAPABILITY = {'current': {'min': .06, 'max': 20.4}}
