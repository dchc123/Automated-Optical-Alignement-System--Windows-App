...
:py:class:`.KeysightN6762AChannel`
::
...
class KeysightN6762AChannel(KeysightN67XXXChannel):
    """
    Keysight N6762A Channel
    """
    ...
    def __init__(self, channel_number, channel_options, interface, dummy_mode, **kwargs):
        ...
        self.voltage = KeysightN6762AVoltBlock(channel_number=channel_number, channel_options=channel_options,
                                               interface=interface, dummy_mode=dummy_mode,
                                               name=self.name+'VoltageMeasure')
        self.current = KeysightN6762ACurrBlock(channel_number=channel_number, channel_options=channel_options,
                                               interface=interface, dummy_mode=dummy_mode,
                                               name=self.name+'CurrentMeasure')


class KeysightN6762AVoltBlock(KeysightN67XXXVoltBlock):
    """
    Keysight N6762A voltage sub block
    """
...
class KeysightN6762ACurrBlock(KeysightN67XXXCurrBlock):
    """
    Keysight N6762A current sub block
    """

    CAPABILITY = {'current': {'min': 0.0001, 'max': 3.06}}
