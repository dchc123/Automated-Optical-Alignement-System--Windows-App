...
:py:class:`.KeysightN6751AChannel`
::
...
class KeysightN6751AChannel(KeysightN67XXXChannel):
    """
    Keysight N6751A Channel
    """
    ...
    def __init__(self, channel_number, channel_options, interface, dummy_mode, **kwargs):
        ...
        self.voltage = KeysightN6751AVoltBlock(channel_number=channel_number, channel_options=channel_options,
                                               interface=interface, dummy_mode=dummy_mode,
                                               name=self.name+'VoltageMeasure')
        self.current = KeysightN6751ACurrBlock(channel_number=channel_number, channel_options=channel_options,
                                               interface=interface, dummy_mode=dummy_mode,
                                               name=self.name+'CurrentMeasure')


class KeysightN6751AVoltBlock(KeysightN67XXXVoltBlock):
    """
    Keysight N6751A voltage sub block
    """

    CAPABILITY = {'voltage': {'min': 0.02, 'max': 51}}
...
class KeysightN6751ACurrBlock(KeysightN67XXXCurrBlock):
    """
    Keysight N6751A current sub block
    """

    CAPABILITY = {'current': {'min': 0.01, 'max': 5.1}}
