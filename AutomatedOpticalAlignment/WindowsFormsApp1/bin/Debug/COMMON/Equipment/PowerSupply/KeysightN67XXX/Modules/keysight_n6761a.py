...
:py:class:`Equipment.PowerSupply.KeysightN67XXX.modules.keysight_n6761a`
::
    ...
    0.102
    >>> ps.channel[1].voltage.protection_level = 40
    >>> ps.channel[1].voltage.setpoint = 50
...
class KeysightN6761AChannel(KeysightN67XXXChannel):
    """
    Keysight N6761A Channel
    """

    CAPABILITY = {'4_wire_sensing': True, 'precision': True}
    ...
    def __init__(self, channel_number, channel_options, interface, dummy_mode, **kwargs):
        ...
        self.voltage = KeysightN6761AVoltBlock(channel_number=channel_number, channel_options=channel_options,
                                               interface=interface, dummy_mode=dummy_mode,
                                               name=self.name+'VoltageMeasure')
        self.current = KeysightN6761ACurrBlock(channel_number=channel_number, channel_options=channel_options,
                                               interface=interface, dummy_mode=dummy_mode,
                                               name=self.name+'CurrentMeasure')


class KeysightN6761AVoltBlock(KeysightN67XXXVoltBlock):
    """
    Keysight N6761A voltage sub block
    """

    CAPABILITY = {'voltage': {'min': 0.012, 'max': 51}}
...
class KeysightN6761ACurrBlock(KeysightN67XXXCurrBlock):
    """
    Keysight N6761A current sub block
    """

    CAPABILITY = {'current': {'min': 0.0001, 'max': 1.53}}
