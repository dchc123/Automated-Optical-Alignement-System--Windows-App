...
For the top level API: See :py:class:`.Keysight81594B`
::

    >>> from CLI.Equipment.OpticalSwitch.Keysight8159XX.keysight_81594b import Keysight81594B
    >>> sw = Keysight81594B('GPIB1::23::INSTR', 1)
    >>> sw.connect()

For channel level API:
:py:class:`.Keysight81594BChannel`
::
    ...
    >>> sw.channel[1].add_state("state2", 2, 1)
    >>> sw.channel[1].states()
    {'state1': [1, 1], 'state2': [2, 1]}
...
class Keysight81594B(Keysight8159XX):
    """
    Keysight 81594B optical switch driver
    """
    ...
    def __init__(self, address, slot_number, interface=None, dummy_mode=False, **kwargs):
        ...
        """:type: list of Keysight81594BChannel"""
        self.channel.append(Keysight81594BChannel(channel_number=1, slot_number=slot_number, interface=interface,
                                                  dummy_mode=dummy_mode))


class Keysight81594BChannel(Keysight8159XXChannel):
    """
    Keysight 81594B optical switch channel
    """

    CAPABILITY = {'in_ports': [1, 2], 'out_ports': [1, 2]}
