...
For the top level API: See :py:class:`.Keithley2001`
::

    >>> from CLI.Equipment.Multimeter.Keithley20XX.keithley_2001 import Keithley2001
    >>> mm = Keithley2001('GPIB1::7::INSTR')
    >>> mm.connect()

For measurement block level API: :py:class:`.Keithley2001AMeasurementBlock`
::
...
class Keithley2001(Keithley20XX):
    """
    Keithley 2001 multimeter driver
    """

    CAPABILITY = {'current_ac_range': {'min': 0, 'max': 2.1},
                  'current_dc_range': {'min': 0, 'max': 2.1},
                  'resistance_2w_range': {'min': 0, 'max': 1050000000},
                  'resistance_4w_range': {'min': 0, 'max': 210000},
                  'volt_ac_range': {'min': 0, 'max': 775},
                  'volt_dc_range': {'min': 0, 'max': 1100}}
    ...
    def __init__(self, address, interface=None, dummy_mode=False, **kwargs):
        ...
        self.current_ac = Keithley2001AMeasurementBlock(type_="CURRent", meas_type_=":AC",
                                                        meas_range_=self.CAPABILITY['current_ac_range'],
                                                        interface=interface, dummy_mode=dummy_mode)
        self.voltage_ac = Keithley2001AMeasurementBlock(type_="VOLTage", meas_type_=":AC",
                                                        meas_range_=self.CAPABILITY['volt_ac_range'],
                                                        interface=interface, dummy_mode=dummy_mode)
        self.current_dc = Keithley2001AMeasurementBlock(type_="CURRent", meas_type_=":DC",
                                                        meas_range_=self.CAPABILITY['current_dc_range'],
                                                        interface=interface, dummy_mode=dummy_mode)
        self.voltage_dc = Keithley2001AMeasurementBlock(type_="VOLTage", meas_type_=":DC",
                                                        meas_range_=self.CAPABILITY['volt_dc_range'],
                                                        interface=interface, dummy_mode=dummy_mode)
        self.resistance_2w = Keithley2001AMeasurementBlock(type_="RESistance", meas_type_="",
                                                           meas_range_= self.CAPABILITY['resistance_2w_range'],
                                                           interface=interface, dummy_mode=dummy_mode)
        self.resistance_4w = Keithley2001AMeasurementBlock(type_="FRESistance", meas_type_="",
                                                           meas_range_=self.CAPABILITY['resistance_4w_range'],
                                                           interface=interface, dummy_mode=dummy_mode)


class Keithley2001AMeasurementBlock(Keithley20XXMeasurementBlock):
    """
    Keithley 2001 multimeter Measurement Block
    """
