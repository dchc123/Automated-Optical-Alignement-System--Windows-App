...
::
    ...
    >>> from COMMON.Equipment.Multimeter.Keysight344XXX.keysight_34410a import Keysight34410A
    >>> mm = Keysight34410A('GPIB1::7::INSTR')
    >>> mm.connect()
...
class Keysight34410A(Keysight344XXX):
    """
    Keysight 34410A multimeter driver
    """

    CAPABILITY = {'current_ac_range': {'min': 0, 'max': 3},
                  'current_dc_range': {'min': 0, 'max': 3},
                  'resistance_2w_range': {'min': 0, 'max': 1000000000},
                  'resistance_4w_range': {'min': 0, 'max': 1000000000},
    ...
    def __init__(self, address, interface=None, dummy_mode=False, **kwargs):
        ...
        self.current_ac = Keysight34410AMeasurementBlock(type_="CURRent", meas_type_=":AC",
                                                         meas_range_=self.CAPABILITY['current_ac_range'],
                                                         interface=interface, dummy_mode=dummy_mode)
        self.voltage_ac = Keysight34410AMeasurementBlock(type_="VOLTage", meas_type_=":AC",
                                                         meas_range_=self.CAPABILITY['volt_ac_range'],
                                                         interface=interface, dummy_mode=dummy_mode)
        self.current_dc = Keysight34410AMeasurementBlock(type_="CURRent", meas_type_=":DC",
                                                         meas_range_=self.CAPABILITY['current_dc_range'],
                                                         interface=interface, dummy_mode=dummy_mode)
        self.voltage_dc = Keysight34410AMeasurementBlock(type_="VOLTage", meas_type_=":DC",
                                                         meas_range_=self.CAPABILITY['volt_dc_range'],
                                                         interface=interface, dummy_mode=dummy_mode)
        self.resistance_2w = Keysight34410AMeasurementBlock(type_="RESistance", meas_type_="",
                                                            meas_range_= self.CAPABILITY['resistance_2w_range'],
                                                            interface=interface, dummy_mode=dummy_mode)
        self.resistance_4w = Keysight34410AMeasurementBlock(type_="FRESistance", meas_type_="",
                                                            meas_range_=self.CAPABILITY['resistance_4w_range'],
                                                            interface=interface, dummy_mode=dummy_mode)


class Keysight34410AMeasurementBlock(Keysight344XXXMeasurementBlock):
    """
    Keysight 34410A multimeter Measurement Block
    """
