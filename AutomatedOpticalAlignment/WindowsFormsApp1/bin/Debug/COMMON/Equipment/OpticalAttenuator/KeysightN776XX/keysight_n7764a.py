...
For the top level API: See :py:class:`.KeysightN7764A`
::

    >>> from CLI.Equipment.OpticalAttenuator.KeysightN776XX.keysight_n7764a import KeysightN7764A
    >>> Att = KeysightN7764A('GPIB1::23::INSTR')
    >>> Att.connect()
    >>> Att.zero_all()

For channel level API:
:py:class:`.KeysightN7764AChannel`
::
...
class KeysightN7764A(KeysightN776XX):
    """
    Keysight N7764A optical attenuator with power control driver
    """

    CAPABILITY = {'channels': 4}
    ...
    def __init__(self, address, interface=None, dummy_mode=False, **kwargs):
        ...
        """:type: list of KeysightN7764AChannel"""
        self.channel.append(KeysightN7764AChannel(channel_number=1, interface=interface, dummy_mode=dummy_mode))
        # Additional channels are on an offset for SCPI commands
        self.channel.append(KeysightN7764AChannel(channel_number=3, interface=interface, dummy_mode=dummy_mode))
        self.channel.append(KeysightN7764AChannel(channel_number=5, interface=interface, dummy_mode=dummy_mode))
        self.channel.append(KeysightN7764AChannel(channel_number=7, interface=interface, dummy_mode=dummy_mode))


class KeysightN7764AChannel(KeysightN776XXChannel):
    """
    Keysight N7764A optical attenuator with power control channel
    """
